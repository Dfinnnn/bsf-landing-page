# Design Brief — Blue Sky Farms (BSF) Landing Page

**Status:** v1.1, historical. **Superseded in parts by PRD v2.0 (public launch):** the real machine is wheeled, the hero image and copy changed, and a mobile layout was added. Read the PRD first.
**Date:** 2026-09-24
...
## 13. Round 2 resolutions (see `prd.md` §3)

- **Brand name:** "Blue Sky Farms" (logo + existing site).
- **Q1 anchoring:** Automate → nozzles; Detect → crop rows; Monitor → flat UI card with no metrics.
- **Q2 company facts:** address is real (21, Jalan SP 3/4, Taman Saujana Puchong, 47100 Puchong, Selangor); legal name, phone and email are placeholders.
- **Q3 logo:** `Logo_BSF.jpg`, untouched; the accent is sampled from the logo's cyan (supersedes the "sky blue" in §5).
- **Q5 land business:** excluded; this page covers technology only.
- **Q6 stack:** plain HTML + CSS, no JS.
- **Q7 stats and partners:** dropped; a non-numeric fact band is optional ("Could").
- **Still open:** see `prd.md` §9.
